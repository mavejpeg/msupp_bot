...
CHANNELS = {
    1: {"id": CHANNEL_1_ID, "name": "Лайфстайл", "link": CHANNEL_1_LINK,
        "sign": f"[Артём хз | Подписаться]({CHANNEL_1_LINK})"},
    2: {"id": CHANNEL_2_ID, "name": "Веб-дизайн", "link": CHANNEL_2_LINK,
        "sign": f"[Arto.ism | Подписаться]({CHANNEL_2_LINK})"},
    3: {"id": CHANNEL_3_ID, "name": "Новости", "link": CHANNEL_3_LINK,
...
async def notify_owner(text: str):
    try:
        ...
        try:
            ...
            logger.error(f"Completely failed to notify owner: {e2}")

async def publish_now(post_data: dict, creator_id: int):
    ch = CHANNELS[post_data["channel"]]
    caption = _build_caption(post_data.get("text", ""), ch["sign"])
    media = post_data.get("media")
    mtype = post_data.get("media_type", "text")
    try:
        if mtype == "text":
        ...
        await notify_owner(f"❌ Ошибка публикации в *{escape_markdown(ch['name'])}*: {escape_markdown(str(e))}")
        raise
...
async def publish_scheduled(post_id: int):
    async with async_session() as session:
        ...
        try:
            ...
            await notify_owner(f"❌ Ошибка публикации запланированного поста (id {post.id}) в *{escape_markdown(ch['name'])}*: {escape_markdown(str(e))}")
